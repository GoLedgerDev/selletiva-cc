...
var CriarSolicitacao = tx.Transaction{
	Tag:         "criarSolicitacao",
	Label:       "Criar Solicitacao",
	Description: "",
	Method:      "POST",
	...
	Routine: func(stub shim.ChaincodeStubInterface, req map[string]interface{}) ([]byte, errors.ICCError) {
		...
		dta_solicita, ok := req["dta_solicita"].(time.Time)
		if !ok {
			return nil, errors.WrapError(nil, "missing argument Data de Solicitacao")
		}
		...
		solicitacao := make(map[string]interface{})
		solicitacao["@assetType"] = "solicitacao"
		solicitacao["id"] = id
		...
		solicitacaoAsset, err := assets.NewAsset(solicitacao)

		_, err = solicitacaoAsset.PutNew(stub)
		if err != nil {
			return nil, errors.WrapError(err, "Error saving vote on blockchain")
		}

		// Marshal asset back to JSON format
		solicitacaoJSON, nerr = json.Marshal(solicitacaoAsset)
		if nerr != nil {
			return nil, errors.WrapError(nil, "failed to encode asset to JSON format")
		}
